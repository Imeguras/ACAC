...
TEST(tst_spac, spac_CheckOdometrySubscription ){
	GTEST_SKIP_("TODO- FIGURE OUT A WAY TO READ CONFIG BEFORE DEPLOYING THIS TEST, ELSE EVEN THOUGH THE PROFILE WORKS TESTING FAILS DUE TO IT NOT BEING PROVIDED A CONFIG FILE");
	//instantiate spac node
	auto ff = std::make_shared<SpacNode>();
	rclcpp::spin_some(ff);
